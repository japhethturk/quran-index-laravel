@extends('layout.site')
@section('title', __('app.index_title_word', ['word_name' => $word->word_name]))
        ...
        <nav class="gradient-45deg-purple-light-blue">
            <div class="nav-wrapper">
              <div class="col s12" style="max-height:3em;">
                <a onclick="window.history.go(-1); return false;" href="" class="breadcrumb">{{$word->letter_name}}</a>
                <a class="breadcrumb">{{$word->word_name}}</a>
              </div>
            </div>
          </nav>
        <div class="card-content" style="padding: 0">
          <div class="collection" style="margin:0">
            @foreach($versesByWords as $verse)
